...
The v2.9.0 harness qualifies authenticated read-only routing to Core GraphQL and Metadata GraphQL after a user creates a local API key through Twenty's supported UI. Its opt-in discovery check also runs the compiled canonical object-metadata query, including v2.9's `ConnectionCursor` paging scalar, and normalizer, requiring a non-empty active standard object with fields without printing workspace identifiers, names, payloads, or counts. It does not yet establish an operational support matrix or exercise CRM lifecycles. See [Local Twenty integration harness](LOCAL_TWENTY.md).
