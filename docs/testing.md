...
- The current documented webhook envelope differs from the pinned v2.9 envelope accepted by the trigger. Current-version trigger compatibility requires a captured disposable delivery and is not claimed.
- `npm run scan:source` checks source and built artifacts before publication. `npm run scan:published` checks registry metadata, provenance, attested source, and downloaded artifacts only after publication and requires explicit success text.
