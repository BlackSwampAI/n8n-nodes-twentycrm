...
The `Record` resource provides `Create`, `Get`, `Get Many`, `Update`, and `Delete` for active standard and custom workspace objects. Friendly `Company` and `Person` resources expose the same operations as metadata-aware wrappers over that shared Record implementation. Their common writable fields appear first in Field Mapping, while all remaining active writable fields, including workspace custom fields, remain available through **Add Field to Send**. The generic Record resource keeps its metadata-defined ordering.
